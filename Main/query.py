...
##MODIFIES: db
##EFFECTS: passes user to webscraper
def scrape(user):
    scholarbot.setParamaters(user)
...
def swipe():
    ...
    swipeframe.place(in_=window, anchor="center", relx=.5, rely=.5)
    ...
    viewmore = Button(swipeframe, font=("Futura", 20), fg="#1874CD", background="white", borderwidth=0,
                      text="View More", command=lambda: callback(url))
    viewmore.grid(row=3, column=1, columnspan=2, pady=5)

    name = Label(ss1, text=result[1], height=4, width=22, wraplength=300, fg="white", borderwidth=2,
                 font="standard-block 20", background="blue").grid(row=0, column=1, sticky='ew')
    amt = Label(ss1, text=result[2], font="standard-block 20", height=4, background="green").grid(row=1, column=1,
                                                                                                  sticky='ew')
    date = Label(ss1, text=result[3], height=4, font="standard-block 20", background="yellow").grid(row=2, column=1,
                                                                                                    sticky='ew')

    right = Button(swipeframe, command=lambda: yah(swipeframe, ssid), fg="#1874CD", text=">>", height=5, width=4,
                   font=("Arial", 45), background="white", borderwidth=0)
    right.grid(row=1, column=4)

    left = Button(swipeframe, text="<<", command=lambda: nah(swipeframe), height=5, fg="#1874CD", width=4,
                  font=("Arial", 45), background="white", borderwidth=0)
